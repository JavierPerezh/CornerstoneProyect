...
async def create_pool(app: FastAPI) -> None:
    ...
    try:
        ...
        logger.debug("Error al crear DB pool: %s", exc)
        raise HTTPException(
...
async def close_pool(app: FastAPI) -> None:
    ...
    try:
        ...
        logger.debug("Error al cerrar DB pool: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Servicio de base de datos no disponible.",
        ) from exc
...
async def get_db(request: Request) -> AsyncGenerator[asyncpg.Connection, None]:
    ...
    try:
        ...
        logger.debug("Error al adquirir/liberar conexión: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Servicio de base de datos no disponible.",
        ) from exc
...
async def execute_query(
    ...
    *args: object,
    ...
    try:
        ...
        logger.debug("Error al ejecutar query: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Servicio de base de datos no disponible.",
        ) from exc
